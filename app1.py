...
/* 🗑️ ゴミ箱ボタンのコンテナ調整 */
[data-testid="stExpander"] [data-testid="column"]:nth-child(2) .stButton {
    display: flex;
...
[data-testid="stExpander"] [data-testid="column"]:nth-child(2) .stButton button {
    ...
    /* Tertiaryボタンでもホバー時に変に色がつかないように微調整 */
    background: transparent !important;
}

/* 🔴 スマホ特有のカラム縦積みを完全ブロック＆【右側の壁から離す】 */
    ...
    with st.expander(f"{cat_info['icon']} **{cat_info['label']}** ({cat_checked}/{cat_total})", expanded=True):
        ...
        for item in cat_items:
            ...
            with col_del:
                ...
                # ★ ここを `type="tertiary"` に変更して枠線を完全に無効化 ★
                st.button("🗑️", type="tertiary", key=del_btn_key, on_click=delete_item, args=(item['id'],), help="削除")
